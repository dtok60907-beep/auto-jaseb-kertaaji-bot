...
type LeaseRow = Readonly<{
  result_status?: "ACQUIRED" | "RENEWED" | "TAKEN_OVER" | "HELD_BY_OTHER";
  account_id: string;
  lease_owner: string;
...
function lease(row: LeaseRow): AccountLease {
...
export class PostgresRuntimeAccountLeaseRepository implements RuntimeAccountLeaseRepository {
  ...
  async acquire(input: Parameters<RuntimeAccountLeaseRepository["acquire"]>[0]): Promise<AccountLeaseAcquisition> {
    const rows = await this.sql<LeaseRow[]>`
      select result_status, account_id::text, lease_owner::text,
             fencing_token::text, lease_until
        from public.acquire_account_lease(
          ${input.accountId}::uuid, ${input.leaseOwner}::uuid, ${input.leaseSeconds}
        )
    `;
    const row = rows[0];
    if (!row?.result_status) throw new Error("ACCOUNT_LEASE_RESULT_MISSING");
  ...
  async renew(input: Parameters<RuntimeAccountLeaseRepository["renew"]>[0]): Promise<AccountLease | null> {
    const rows = await this.sql<LeaseRow[]>`
      select account_id::text, lease_owner::text, fencing_token::text, lease_until
        from public.renew_account_lease(
          ${input.accountId}::uuid, ${input.leaseOwner}::uuid,
          ${input.fencingToken.toString()}::bigint, ${input.leaseSeconds}
        )
    `;
  ...
  async release(input: Parameters<RuntimeAccountLeaseRepository["release"]>[0]): Promise<boolean> {
    const rows = await this.sql<{ released: boolean }[]>`
      select public.release_account_lease(
        ${input.accountId}::uuid, ${input.leaseOwner}::uuid,
