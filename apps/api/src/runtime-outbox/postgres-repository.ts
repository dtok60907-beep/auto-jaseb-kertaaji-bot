...
type ClaimedRow = { command_id: string; operation_id: string; account_id: string; kind: ClaimedWorkflowCommand["kind"]; target_id: string; payload: Record<string, unknown>; fencing_token: string; lease_until: Date | string };
function claimed(row: ClaimedRow): ClaimedWorkflowCommand { return Object.freeze({ id: row.command_id, operationId: row.operation_id, accountId: row.account_id, kind: row.kind, targetId: row.target_id, payload: Object.freeze(row.payload), fencingToken: BigInt(row.fencing_token), leaseUntil: new Date(row.lease_until).toISOString() }); }
